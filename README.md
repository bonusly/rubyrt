...
The approval comment lists the checks that passed and a collapsed **RubyRT details** section (RubyRT version and the review model). When an LLM key is available in the `github-comment` step, it also adds an informational **risk assessment** (a Low/Medium/High level and a short summary of what passed and any residual risk). The risk level never affects the decision — the deterministic rules above do — and the section is simply omitted if no LLM client is configured or the call fails.
