...
RSpec.describe Rubyrt::CLI do
  ...
  def run_models(argv)
    Rubyrt::CLI.start(['models', *argv])
  end

  context 'when no models_file is configured' do
    let(:config) { Rubyrt::Configuration.new(root: tmp_dir, overrides: { 'models_file' => '' }) }

    before do
      allow(Rubyrt::Configuration).to receive(:new).and_return(config)
  ...
  context 'when --path is given' do
    ...
    let(:config) { Rubyrt::Configuration.new(root: tmp_dir, overrides: { 'models_file' => models_path }) }
